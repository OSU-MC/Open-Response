...
function Lectures(props) {
    ...
    const fetchLectures = async () => {
      try {
        ...
        const response = await apiUtil(
          "get",
          `/courses/${courseId}/sections/${user.user.id}/lectures/live`,
  ...
  return (
    <div className="lectures">
      <div className="lectures-top-bar">
        ...
        {role !== "teacher" && liveLecture && (
          <Link
            ...
            to={`/${courseId}/live/${liveLecture.id}`}
          >
            <Button variant="success" className="btn-add">
              {liveLecture.name} is Live
            </Button>
          </Link>
        )}
      </div>
      ...
      {role !== "student" && (
        <Button className="create-lecture-btn" onClick={openCreateModal}>
          + Create Lecture
        </Button>
      )}
    </div>
