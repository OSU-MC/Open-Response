...
function Questions(props) {
  ...
  const [questions, nextLink, prevLink, message, error, loading] =
    useQuestions();
  ...
  const addQuestionsToLecture = (e) => {
    ...
    Object.values(lecture.staged).forEach(async (question) => {
      ...
      let response = await apiUtil(
        "post",
        `courses/${courseId}/lectures/${lectureId}/questions/${question.id}`,
  ...
  return (
    <>
      {questionView != null && (
        <Popup close={closeQuestion}>
          <SingleQuestionTeacher question={questionView} editable={true} />
        </Popup>
      )}
      <div className="contentView">
        <div className="header">
          <button className="btn btn-secondary">
            ...
            >
              &lt;&lt; Back to {lectureId ? "Lecture" : "Course"}
            </NavLink>
          </button>
          {(submissionPosted || submissionErrors.length > 0) && (
            <Notice
              message={
                submissionErrors.length > 0
                  ? submissionErrors[0]
                  : "Questions added to lecture"
              }
              status={submissionErrors.length > 0 ? "error" : "success"}
            />
          )}
          {lectureId &&
            (submissionLoading > 0 ? (
              ...
              <button
                ...
                onClick={(e) => addQuestionsToLecture(e)}
              >
                Submit
              </button>
            ))}
        ...
        <div className="searchBarContainer">
          ...
          >
            ...
            <div className="split-space">
              <button className="dynamic-btn btn btn-primary" type="submit">
                Search
              </button>
              <div>&nbsp;</div>
              <button
                className="dynamic-btn btn btn-add"
                onClick={(event) => event.preventDefault()}
              >
                <NavLink className="basicLink" to="add">
                  Create Question
                </NavLink>
              </button>
            </div>
        ...
        <div className="questionsContainer">
          {loading ? (
            ...
            questions[courseId].length > 0 ? (
                ...
                {questions[courseId].map((question) => {
                  return (
                    <QuestionListItem
                      key={question.id}
                      question={question}
                      selectable={lectureId}
                      staged={
                        lecture.staged[question.id] != null ? true : false
                      }
                      onClick={(e) => {
              ...
              <Notice message={"No questions found"} />
            )
          ) : (
            <></>
          )}
