...
function Profile(props) {
  ...
  useEffect(() => {
    const getUserInfo = async () => {
      setLoading(true);
      const response = await apiUtil("get", `/users/${userState.user.id}`, {
        dispatch: dispatch,
        navigate: navigate,
      });
      setLoading(false);
  ...
  function sendEditedAccountInfo() {
    ...
    if (currentPassword != "") newAccountInfo.oldPassword = currentPassword;
    if (newPassword != "") newAccountInfo.rawPassword = newPassword;
        ...
        {loading ? (
          ...
          <Container className="profileInfo">
            <ListGroup className="tester">
              ...
              <Col xs={1} className="profileButton">
                {" "}
                <Button onClick={onClickFunctions}>
                  {" "}
