...
function StudentGradebook({ grades }) {
  const lectureGrades = Array.isArray(grades)
    ? grades
    : Array.isArray(grades?.lectures)
      ? grades.lectures
      : [];
        ...
        <tbody>
          {lectureGrades.map((lecture) => (
            <tr key={lecture.lectureId}>
              ...
              <td className="grades-questions">
                {lecture.totalAnswered} / {lecture.totalQuestions}
              </td>
              <td className="grades-grade">
