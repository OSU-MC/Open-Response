  ...
  useEffect(() => {
    async function getResponses() {
      ...
      const response = await apiUtil(
        "get",
        `courses/${courseId}/sections/${course.Sections[0].id}/lectures/${lectureId}/responses`,
        { dispatch: dispatch, navigate: navigate }
      );
      setMessage(response.message);
      setError(response.error);
      if (response.status === 200) {
        const responseForQuestion = response.data.filter(
          (obj) => obj.question.id == questionId
        );
