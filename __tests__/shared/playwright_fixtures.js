...
export const test = base.extend({
  ...
  profileEditPage: async ({ profilePage }, use) => {
    const editProfileButton = profilePage.locator(
      "div.profileButton >> button"
    );
  ...
  coursePageStudent: async ({ loggedInPageStudent }, use) => {
    const recentCoursesSection = loggedInPageStudent
      .locator("#courses-page >> .course-section")
      .filter({
  ...
  lecturePageStudent: async ({ coursePageStudent }, use) => {
    const firstLectureCard = coursePageStudent
      .locator("div.lectures-container >> div.lecture-card")
      .first();
  ...
  questionInLecturePageStudent: async ({ lecturePageStudent }, use) => {
    const firstQuestionCard = lecturePageStudent
      .locator("div.question-card-student")
      .first();
  ...
  coursePageInstructor: async ({ loggedInPageInstructor }, use) => {
    const recentCoursesSection = loggedInPageInstructor
      .locator("#courses-page >> .course-section")
      .filter({
        has: loggedInPageInstructor.locator("h2", {
          hasText: "Recent Courses",
        }),
      });
  ...
  courseCreateSectionModalInstructor: async (
  ...
  courseCreateLectureModalInstructor: async (
  ...
  sectionLecturePageInstructor: async ({ sectionPageInstructor }, use) => {
    const firstSectionCard = sectionPageInstructor
      .locator("div.section-card")
      .first();
    await firstSectionCard
      .locator("div.card >> div.card-body")
      .getByText("View Section")
      .click();
  ...
  addLecturetoSectionModalInstructor: async (
  ...
  sectionGradeBookPageInstructor: async ({ sectionPageInstructor }, use) => {
    const gradebookLink = sectionPageInstructor
      .locator("div.tabs >> a")
      .filter({
        has: sectionPageInstructor.locator("span.no-link-style", {
          hasText: "Gradebook",
        }),
      });
  ...
  viewLecturePageInstructor: async ({ sectionLecturePageInstructor }, use) => {
    const firstLectureCard = sectionLecturePageInstructor
      .locator("div.horizontal-flex-container >> div.lecture-card")
      .first()
      .getByText("View Lecture");
    await firstLectureCard.click();

    await sectionLecturePageInstructor.waitForURL(
      /\/\d+\/sections\/\d+\/lectures\/\d+/
    );

    await use(sectionLecturePageInstructor); // navigated to /:courseId/sections/:sectionId/lectures/:lectureId
  },

  lectureTemplateEditPageInstructor: async (
    { lectureTemplatesPageInstructor },
    use
  ) => {
  ...
  lectureTemplateEditAddQuestionPageInstructor: async (
    ...
    await lectureTemplateEditPageInstructor.waitForURL(
      /\/\d+\/lectures\/\d+\/questions\/add/
    );
    await use(lectureTemplateEditPageInstructor); // navigated to /:courseId/lectures/:lectureTemplateId/questions/add
  },

  lectureTemplateEditQuestionEditPageInstructor: async (
    { lectureTemplateEditPageInstructor },
    use
  ) => {
