...
router.post("/", requireAuthentication, async function (req, res, next) {
  try {
    ...
    if (!enrollmentStudent) {
      return res
        .status(403)
        .send({ error: "Only a student in the course can submit a response" });
    }
    ...
    if (!questionInLecture) {
      return res.status(404).send({
        error:
          "No question in lecture found for the given lecture and question",
      });
    }
    ...
    if (!studentGrade) {
      const newGrade = {
        ...
        points: pointsForThisQuestion,
        totalPoints: totalPointsForThisQuestion,
        grade: totalPointsForThisQuestion
          ? pointsForThisQuestion / totalPointsForThisQuestion
          : 0,
        lectureForSectionId: lectureForSection.id,
      };
      ...
      const newGradeValue = studentGrade.totalPoints
        ? studentGrade.points / studentGrade.totalPoints
        : 0;
...
router.put(
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      const question = await questionService.getQuestionInCourse(questionId);
      let totalCorrectWeight = 0;
      let correctPoints = 0;
      let extraPenalty = 0;
