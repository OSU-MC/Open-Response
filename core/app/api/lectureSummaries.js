...
router.get("/", requireAuthentication, async function (req, res, next) {
    ...
    try {
      ...
      for (let i = 0; i < questionsInLecture.length; i++) {
        const question = await db.Question.findByPk(
          questionsInLecture[i].questionId
        );
        ...
        responses.length !== 0
          ? (questionArrayObj.averageScore = parseFloat(
              (sum / responses.length).toFixed(2)
            ))
          : (questionArrayObj.averageScore = 0);
  ...
  // if request is not by teacher or qualified student
    ...
    res.status(403).send({
      error:
        "User is neither the teacher for the course nor a student in the correct section of the course",
    });
  }
});
