...
router.get("/", requireAuthentication, async function (req, res) {
  ...
  const enrollment =
    (await getEnrollmentFromCourse(user.id, courseId)) ||
    (await getEnrollmentFromSectionInCourse(user.id, courseId));
  const course = await getCourse(courseId);

  if (enrollment == null) {
    // if user is not enrolled in this course
    res.status(403).send();
  } else if (enrollment.role == "student" && !course.published) {
    // if user is a student, and course isn't published, return 'No Content'
    res.status(204).send();
  }
  // if student and published get all lectures that are published their section
  else if (enrollment.role == "student" && course.published) {
    // if user is a student, and course is published
    ...
    if (lecturesForSection.length === 0) {
      // if no lectures are published for this section
      res.status(204).send();
    } else {
      res.status(200).json({
    ...
    // if teacher, OR student in published course
    ...
    if (lectures == []) {
      // if no lectures are in this course
      res.status(204).send();
    } else {
      res.status(200).json({
        lectures: lectures,
...
router.post("/", requireAuthentication, async function (req, res) {
  ...
  if (enrollment == null) {
    ...
    if (missingFields.length == 0) {
      try {
        ...
        if (e instanceof UniqueConstraintError) {
          return res.status(400).send({
            error:
              "There exists a lecture with this order number in this course",
          });
      ...
      return res
        .status(400)
        .send({ error: `Missing required lecture fields: ${missingFields}` });
    }
...
router.put("/:lecture_id", requireAuthentication, async function (req, res) {
  ...
  if (lecture == null) {
    // if passed in lectureId does not exist
    ...
    try {
      ...
      if (e instanceof UniqueConstraintError) {
        return res.status(400).send({
          error: "There exists a lecture with this order number in this course",
        });
      } else if (e instanceof ValidationError) {
        return res.status(400).send({ error: serializeSequelizeErrors(e) });
      } else {
        return res
          .status(400)
          .send({ error: "Unable to update lecture object" });
      }
    }
...
router.get("/:lecture_id", requireAuthentication, async function (req, res) {
  try {
    ...
    if (enrollment == null) {
      ...
      return res
        .status(404)
        .send({ error: "Lecture of this id does not exist" });
    }
      ...
      try {
        ...
        if (e instanceof ValidationError) {
          ...
          return res.status(500).send({
            error: "Unable to fetch lecture or questions for student",
          });
        }
      }
    }
    try {
      ...
      if (e instanceof ValidationError) {
        ...
        return res
          .status(500)
          .send({ error: "Unable to fetch lecture or questions" });
      }
    }
