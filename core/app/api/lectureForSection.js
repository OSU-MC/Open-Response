...
router.get("/live", requireAuthentication, async function (req, res, next) {
  ...
  try {
    ...
    if (!sectionData) {
      return res
        .status(403)
        .send({ error: "User is not enrolled in this course." });
    }
    ...
    const liveLectureSections = lectureForSections.filter(
      (lfs) => lfs.dataValues.isLive
    );

    if (lectureForSections.length === 0) {
      return res
        .status(404)
        .send({ error: "No live lectures found in this course and section." });
    }
...
router.get("/", requireAuthentication, async function (req, res, next) {
  ...
  try {
    ...
    if (!isTeacher) {
      return res
        .status(403)
        .send({ error: "Must be a teacher of this course to view lectures" });
    }
    ...
    if (!section) {
      return res
        .status(404)
        .send({ error: "Section not found or does not belong to the course" });
    }
      ...
      if (isTeacher) {
        ...
        if (foundSection) {
          if (foundSection.courseId == courseId) {
            const foundLectureInSection =
              await lectureService.getLectureForSection(sectionId, lectureId);
            if (foundLectureInSection) {
              ...
              await foundLectureInSection.update({
                published: updatePublishedTo,
              });
              res.status(200).send();
            } else {
              // no relationship between given lecture and section
              res.status(400).send({
                error: `No relationship between lecture of id ${lectureId} and section of id ${sectionId}`,
              });
            }
          } else {
            // given section is not in this course
            res.status(400).send({
              error: `Section of id ${sectionId} is not in course of id ${courseId}`,
            });
          }
        } else {
          ...
          res
            .status(404)
            .send({ error: `No section of id ${sectionId} was found` });
        }
      } else {
        // user is not a teacher
        res.status(403).send({
          error: "Must be a teacher of this course to publish lecture",
        });
      }
    } catch (e) {
      next(e);
    ...
    try {
      ...
      if (!isTeacher) {
        return res.status(403).send({
          error:
            "Must be a teacher of this course to get questions from a lecture",
        });
      }
      ...
      if (!section) {
        return res.status(404).send({
          error: "Section not found or does not belong to the course",
        });
      }
      ...
      if (!lecture) {
        return res.status(404).send({
          error: "Lecture not found or does not belong to the course",
        });
      }
      ...
      const questionIds = questionInLectureRecords.map(
...
router.post("/", requireAuthentication, async function (req, res, next) {
  ...
  if (
    ...
    lectureWeight === undefined ||
    lectureWeight === null
  ) {
    return res.status(400).send({
      error: "Missing required parameters: courseId, sectionId, or lectureId",
    });
  }
  ...
  try {
    ...
    if (!isTeacher) {
      return res.status(403).send({
        error: "Must be a teacher of this course to add a lecture to a section",
      });
    }
    ...
    if (!lecture) {
      return res
        .status(404)
        .send({ error: "Lecture not found or does not belong to the course" });
    }
    ...
    if (existingLectureForSection) {
      return res
        .status(400)
        .send({ error: "Lecture already exists in this section" });
    }
...
router.delete(
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      res
        .status(200)
        .send({ message: "LectureForSection deleted successfully" });
    } catch (error) {
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      if (isTeacher) {
        ...
        if (lectureForSection) {
          await lectureForSection.update({
            isLive: liveStatus,
            published: true,
          });
        ...
        res
          .status(403)
          .send({ error: "User must be a teacher to update the lecture" });
      }
    } catch (e) {
      next(e);
    }
