...
router.get(
  ...
  async function (req, res, next) {
    try {
      ...
      const isTeacher = await enrollmentService.checkIfTeacher(
        user.id,
        courseId
      );
      if (!isTeacher) {
        return res.status(403).send({
          error: "Must be a teacher of this course to get question info",
        });
      }

      const isLecInCourse = await lectureService.getLectureInCourse(
        lectureId,
        courseId
      );
      if (!isLecInCourse) {
        return res.status(400).send({
          error: "The given lecture ID does not belong to this course",
        });
      }
      ...
      if (!lectureForSection) {
        ...
        return res.status(400).send({
          error: "The given lecture ID does not belong to this section",
        });
      }

      const questionInLecture = await questionService.getQuestionInLecture(
        questionId,
        lectureForSection.id
      );
      if (!questionInLecture) {
        return res.status(404).send({
          error: "The given question ID does not belong to this lecture",
        });
      }
      ...
      if (!question) {
        ...
        return res
          .status(404)
          .send({ error: "The given question ID not found in this course" });
      }
...
router.put(
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      const isLecInSection = await lectureService.getLectureForSection(
        sectionId,
        lectureId
      );
      if (!isLecInSection) {
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      if (!questionInLecture) {
        return res
          .status(400)
          .send({ error: "Question ID does not belong to this lecture" });
      }
      ...
      if (!question) {
        return res
          .status(404)
          .send({ error: "Question ID not found in this course" });
      }
      ...
      if (isTeacher) {
        ...
        if (isLecInCourse) {
          ...
          if (question && lectureForSection) {
            ...
            const missingFields =
              questionService.validateQuestionInLectureCreationRequest(
                newQsLecRelation
              );
            if (missingFields.length == 0) {
              ...
              res
                .status(201)
                .send(
                  questionService.extractCompleteQuestionInLectureFields(
                    newQsLecRelation
                  )
                );
            } else {
              return res
                .status(400)
                .send({ error: `Missing required fields: ${missingFields}` });
            }
          } else {
            // if there's no question of this id (from this course) or lectureForSection not found
            return res.status(404).send({
              error:
                "The given question ID not found in this course or lectureForSection not found",
            });
          }
        } else {
          // if given lecture is not in this course
          return res.status(400).send({
            error: "The given lecture ID does not belong to this course",
          });
        }
        ...
        return res.status(403).send({
          error: "Must be a teacher of this course to link question to lecture",
        });
      }
    } catch (e) {
      next(e);
    ...
    if (req.body.questionIdOne && req.body.questionIdTwo) {
      ...
      if (isTeacher) {
        ...
        if (isLecInCourse) {
          ...
          const questionOneInLecture =
            await questionService.getQuestionInLecture(
              questionIdOne,
              lectureForSection.id
            );
          const questionTwoInLecture =
            await questionService.getQuestionInLecture(
              questionIdTwo,
              lectureForSection.id
            );
          if (questionOneInLecture && questionTwoInLecture) {
            // check if both questions have a relationship with this lecture
            const questionOne = await questionService.getQuestionFromLecture(
              questionIdOne,
              lectureId
            );
            const questionTwo = await questionService.getQuestionFromLecture(
              questionIdTwo,
              lectureId
            );
            if (questionOne && questionTwo) {
              // check that both questions exist in this course
              ...
              // if either of the question ids was not found (in this course)
              res.status(404).send({
                error:
                  "One (or both) of the given question IDs not found in this course",
              });
            }
          } else {
            // if either of the given questions are not in this lecture
            res.status(400).send({
              error:
                "One (or both) of the provided question IDs is not in this lecture",
            });
          }
        } else {
          // if given lecture is not in this course
      ...
      // if either of the questionIds was not passed in the body
      res.status(400).send({
        error:
          "Must provide 'questionIdOne' and 'questionIdTwo' in request body",
      });
    }
  } catch (e) {
    console.error(e);
  ...
  async function (req, res, next) {
    ...
    try {
      ...
      if (!isTeacher) {
        return res.status(403).send({
          error:
            "Must be a teacher of this course to remove question from lecture",
        });
      }
