...
async function getLecturesFromSection(sectionId) {
  ...
  for (let i = 0; i < foundLectures.length; i++) {
    if (foundLectures[i].LectureForSections.length == 1) {
      // SHOULD always be true, given query above
      foundLectures[i].dataValues["published"] =
        foundLectures[i].LectureForSections[0].published; // bring published attr up
      foundLectures[i].dataValues["isLive"] =
        foundLectures[i].LectureForSections[0].isLive;
...
router.post("/", requireAuthentication, async function (req, res, next) {
  ...
  if (req.body.number && isTeacher) {
    // if course number was passed in, and user is teacher
    ...
    try {
      ...
      if (e instanceof UniqueConstraintError) {
        res.status(400).send({
          error:
            "A section in this course with this section number already exists",
        });
    ...
    if (isTeacher) {
      // user is teacher, problem is that number wasn't passed in
      res.status(400).send({
        error: `Request did not contain required fields to create a section`,
      });
    } else if (req.body.number) {
      // number is passed in, problem is that user isn't teacher
      res
        .status(403)
        .send({ error: `Only the teacher for a course can create a section` });
    } else {
      res.status(403).send({
        error: `Request did not contain required fields to create a section and user does not have credentials to do so anyways`,
      });
    }
  }
});
  ...
  if (isTeacher) {
    try {
      ...
      if (sectionsInCourse.length != 0) {
        // if sections are present for this course
    ...
    res
      .status(403)
      .send({ error: "Only the teacher for a course can get a section" });
  }
});
    ...
    if (isTeacher) {
      try {
        ...
        if (foundSection != null) {
          // if section was found for given id
          if (foundSection.courseId == courseId) {
            ...
            res.status(400).send({
              error: "This section does not belong to the given course",
            });
          }
        } else {
          res
            .status(404)
            .send({ error: "There is no section for the provided section id" });
        }
      } catch (e) {
        next(e);
      }
      ...
      res.status(403).send({
        error: "Must be a teacher of this course to get section info",
      });
    }
  }
);
...
router.put(
  ...
  async function (req, res, next) {
    ...
    if (isTeacher) {
      if (foundSection != null) {
        if (foundSection.courseId == courseId) {
          if (newNumber != null && newNumber != "") {
            // number is the only field that can be updated, so enforce that it is present and valid
            try {
            ...
            res.status(400).send({
              error:
                "Must enter a valid number field. Number is the only field that can be updated",
            });
          }
        } else {
          res.status(400).send({
      ...
      res
        .status(403)
        .send({ error: "Must be a teacher of this course to update section" });
    }
  }
);
...
router.use(
  "/:section_id/lectures/:lecture_id/questions",
  require("./questionsInLecture")
);
