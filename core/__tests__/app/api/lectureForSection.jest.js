...
describe("Test api/lecturesForSection", () => {
  ...
  beforeAll(async () => {
    ...
    const maxOrder =
      (await db.Lecture.max("order", { where: { courseId: course1.id } })) || 1;
  ...
  describe("PUT /courses/:course_id/sections/:section_id/lectures/:lecture_id", () => {
    it("should respond with 403 for updating as a student", async () => {
      const resp = await request(app)
        .put(
          `/courses/${course1.id}/sections/${section1.id}/lectures/${lecture1.id}`
        )
    ...
    it("should respond with 404 for updating a section id that does not exist", async () => {
      const resp = await request(app)
        .put(`/courses/${course1.id}/sections/-1/lectures/${lecture1.id}`)
    ...
    it("should respond with 400 for updating a section that is not in this course", async () => {
      ...
      const resp = await request(app)
        .put(
          `/courses/${temp_course.id}/sections/${section1.id}/lectures/${lecture1.id}`
        )
        .set("Cookie", teachCookies);
    ...
    it("should respond with 400 for updating when there is no relationship between section and lecture", async () => {
      ...
      const resp = await request(app)
        .put(
          `/courses/${course1.id}/sections/${section1.id}/lectures/${tempLec.id}`
        )
        .set("Cookie", teachCookies);
  ...
  describe("GET /courses/:course_id/sections/:section_id/lectures/:lecture_id/questions", () => {
    it("should respond with 403 for fetching questions as a student", async () => {
      const resp = await request(app)
        .get(
          `/courses/${course1.id}/sections/${section1.id}/lectures/${lecture1.id}/questions`
        )
        .set("Cookie", studentCookies);
    ...
    it("should respond with 404 for fetching questions from a non-existent section", async () => {
      const resp = await request(app)
        .get(
          `/courses/${course1.id}/sections/-1/lectures/${lecture1.id}/questions`
        )
        .set("Cookie", teachCookies);
    ...
    it("should respond with 404 for fetching questions from a lecture not in the course", async () => {
      ...
      const resp = await request(app)
        .get(
          `/courses/${course1.id}/sections/${section1.id}/lectures/${tempLecture.id}/questions`
        )
        .set("Cookie", teachCookies);
  ...
  describe("POST /courses/:course_id/sections/:section_id/lectures", () => {
    ...
    it("should create lectureForSection, questionsInLecture, and lectureGradeWeight (int weight)", async () => {
      ...
      let lfs = await db.LectureForSection.findOne({
        where: { sectionId: section1.id, lectureId: lecture2.id },
      });
      expect(lfs).not.toBeNull();
      let qil = await db.QuestionInLecture.findAll({
        where: { lectureForSectionId: lfs.id },
      });
      expect(qil.length).toBe(1);
      let weight = await db.LectureGradeWeight.findOne({
        where: { LectureForSectionId: lfs.id },
      });
    ...
    it("should create lectureForSection, questionsInLecture, and lectureGradeWeight (float weight)", async () => {
      ...
      expect(resp.statusCode).toBe(200);
      let lfs = await db.LectureForSection.findOne({
        where: { sectionId: section2.id, lectureId: lecture2.id },
      });
      let weight = await db.LectureGradeWeight.findOne({
        where: { LectureForSectionId: lfs.id },
      });
    ...
    it("should create lectureForSection, questionsInLecture, and lectureGradeWeight (zero weight)", async () => {
      const section3 = await db.Section.create({
        number: 100,
        joinCode: "zero00",
        courseId: course1.id,
      });
      ...
      expect(resp.statusCode).toBe(200);
      let lfs = await db.LectureForSection.findOne({
        where: { sectionId: section3.id, lectureId: lecture2.id },
      });
      let weight = await db.LectureGradeWeight.findOne({
        where: { LectureForSectionId: lfs.id },
      });
