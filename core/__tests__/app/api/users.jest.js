...
describe("POST /users", () => {
  ...
  it("should respond with 400 and missing fields", async () => {
    ...
    expect(resp.body.error).toEqual(
      "Missing fields required to create user: lastName, isTeacher, confirmedPassword"
    );
  });
...
describe("POST /users/login", () => {
  ...
  it("should respond with 404 and message that email does not exist", async () => {
    ...
    expect(resp.body.error).toEqual(
      "No account found with email: abc123@open-response.org"
    );
  });
  ...
  it("should respond with 401 and message that account is locked", async () => {
    ...
    expect(resp.body.error).toEqual(
      "This account has been locked until the password is reset. An email should have been sent with instructions"
    );
    await user.destroy();
  });
  ...
  it("should respond with 401 and message that account is now locked", async () => {
    ...
    expect(resp.statusCode).toEqual(401);
    expect(resp.body.error).toEqual(
      "Incorrect password. Your account has been locked. You will need to reset your password before logging in. An email should have been sent to your inbox"
    );
    await user.reload();
  ...
  it("should respond with 401 and message that account has 2 remaining tries", async () => {
    ...
    expect(resp.statusCode).toEqual(401);
    expect(resp.body.error).toEqual(
      "Incorrect password. Your account will be locked after 2 more unsuccessful attempts"
    );
    await user.reload();
...
describe("PUT /users", () => {
  ...
  it("should respond with 200 and reset the password successfully", async () => {
    ...
    expect(
      moment(user.lastLogin).isBetween(
        moment().subtract(1, "seconds").utc(),
        moment().add(1, "seconds").utc()
      )
    );
  });
  ...
  it("should respond with 400 and missing fields for request", async () => {
    ...
    expect(resp.statusCode).toEqual(400);
    expect(resp.body.error).toEqual(
      "Missing fields required to reset password: passwordResetCode"
    );
  });
  ...
  it("should respond with 401 and credentials expired", async () => {
    ...
    expect(resp.statusCode).toEqual(401);
    expect(resp.body.error).toEqual(
      "Credentials expired. Please request new credentials and try again"
    );
  });
...
describe("PUT /users/password", () => {
  ...
  it("should respond with 204 and initiate password reset request", async () => {
    ...
    expect(resp.statusCode).toEqual(200);
    expect(resp.body.message).toEqual(
      `If an account exists for ${user.email}, password reset instructions will be sent`
    );
    await user.reload();
  ...
  it("should respond with 204 and not initiate reset for non-existent email", async () => {
    ...
    expect(resp.statusCode).toEqual(200);
    expect(resp.body.message).toEqual(
      `If an account exists for thisemaildoesntexist@open-response.org, password reset instructions will be sent`
    );
  });
  ...
  it("should respond with 400 and missing fields", async () => {
    ...
    expect(resp.statusCode).toEqual(400);
    expect(resp.body.error).toEqual(
      "Missing fields required for password reset request: email"
    );
  });
...
describe("/users/:userId", () => {
  ...
  beforeAll(async () => {
    ...
    userCookies = [
      `_openresponse_session=${userJwt}`,
      `xsrf-token=${userSession.csrfToken}`,
    ];
    adminCookies = [
      `_openresponse_session=${adminJwt}`,
      `xsrf-token=${adminSession.csrfToken}`,
    ];
  });
  ...
  describe("GET", () => {
    ...
    it("should return 403 and insufficient permissions", async () => {
      ...
      expect(resp.body.error).toEqual(
        `Insufficient permissions to access that resource`
      );
    });
  ...
  describe("PUT", () => {
    ...
    it("should respond with 400 and no fields", async () => {
      ...
      expect(resp.statusCode).toEqual(400);
      expect(resp.body.error).toEqual(
        `Missing any valid fields to update the user: email, firstName, lastName`
      );
    });
    ...
    it("should respond with 400 and missing fields for password change", async () => {
      ...
      expect(resp.statusCode).toEqual(400);
      expect(resp.body.error).toEqual(
        `Missing fields required to update password: confirmedPassword`
      );
    });
    ...
    it("should respond with 401 and incorrect password", async () => {
      ...
      expect(resp.statusCode).toEqual(401);
      expect(resp.body.error).toEqual(
        `Incorrect password. Cannot set new password.`
      );
    });
  ...
  describe("PUT /confirm", () => {
    ...
    it("should respond with 498 and emailConfirmationCode expired and new email send", async () => {
      ...
      expect(resp.body.error).toEqual(
        `emailConfirmationCode expired. A new code should have been emailed.`
      );
      await user.reload();
