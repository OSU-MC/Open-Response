...
describe("/courses endpoints", () => {
  ...
  it("should respond with 201 when a valid course is created and should create an enrollment", async () => {
    ...
    expect(resp.body.course.description).toEqual(
      "Wanna get lit? We'll show you how"
    );
  ...
  it("should respond with 201 when a valid section is created", async () => {
    const respSection = await request(app)
  ...
  it("should respond with 200 and the teacher courses enrolled in", async () => {
    ...
    expect(resp.body.teacherCourses[1].description).toEqual(
      "Wanna get lit? We'll show you how"
    );
  });

  it("should respond with 200 and the student courses enrolled in", async () => {
    const respStudent = await request(app)
      .get("/courses")
      .set("Cookie", user2Cookies);
    ...
    expect(respStudent.body.studentCourses[0].description).toEqual(
      "Wanna get lit? We'll show you how"
    );
    expect(respStudent.body.studentCourses[0].Sections[0].id).toEqual(
      section.id
    );
    expect(respStudent.body.studentCourses[0].Sections[1].id).toEqual(
      section2.id
    );
  });
  ...
  it("should respond with 200 when teacher tries to edit a course", async () => {
    const resp = await request(app)
      ...
      .set("Cookie", userCookies);
    expect(resp.statusCode).toEqual(200);
    expect(resp.body.course.name).toEqual(
      "Willy Wonka and his darn chocolate factory"
    );
    expect(resp.body.course.description).toEqual(
      "I've got a golden ticket dun dun dun dun dun"
    );
  ...
  it("should respond with 200 when teacher tries to edit a course with only some fields", async () => {
    ...
    expect(resp.body.course.description).toEqual(
      "This is a description. No, like seriously it is"
    );
    expect(resp.body.course.published).toEqual(true);
  });
