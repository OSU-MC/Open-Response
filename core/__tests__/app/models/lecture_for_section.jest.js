...
describe("LectureForSection model", () => {
  ...
  describe("LectureForSection.create", () => {
    ...
    it("should reject a LectureForSection without a lectureId", async () => {
      await expect(
        ...
        "notNull Violation: Lecture For Section must have a lecture"
      );
    });
    ...
    it("should reject a LectureForSection without a sectionId", async () => {
      await expect(
        ...
        "notNull Violation: Lecture For Section must have a section"
      );
    });
    ...
    it("should reject a LectureForSection without an attendanceMethod", async () => {
      await expect(
        ...
        "notNull Violation: LectureForSection.attendanceMethod cannot be null"
      );
    });
    ...
    it("should reject an invalid attendanceMethod value", async () => {
      await expect(
        ...
        "Lecture For Section must have a valid attendance method"
      );
    });
  ...
  describe("LectureForSection.update", () => {
    ...
    it("should update publishedAt timestamp", async () => {
      ...
      const updatedLfs = await db.LectureForSection.findOne({
        where: { id: lfs.id },
      });

      expect(updatedLfs.publishedAt).not.toBeNull();
      expect(
        moment(updatedLfs.publishedAt).utc().isSame(publishedAt, "second")
      ).toBe(true);
    });
    ...
    it("should update closeAttendanceAt timestamp", async () => {
      ...
      expect(
        moment(updatedLfs.closeAttendanceAt)
          .utc()
          .isSame(closeAttendanceAt, "second")
      ).toBe(true);
    });
    ...
    it("should update closedAt timestamp", async () => {
      ...
      expect(moment(updatedLfs.closedAt).utc().isSame(closedAt, "second")).toBe(
        true
      );
    });
    ...
    it("should reject invalid attendanceMethod values", async () => {
      await expect(
        lfs.update({ attendanceMethod: "invalidMethod" })
    ...
    it("should reject negative minAttendanceQuestions", async () => {
      await expect(lfs.update({ minAttendanceQuestions: -1 })).rejects.toThrow(
        ValidationError
      );
    });
