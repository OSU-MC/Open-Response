...
describe("Course model", () => {
  ...
  describe("Course.create", () => {
    it("should create a valid course record with default values", async () => {
      const course = await db.Course.create({
        ...
        description:
          "An introduction to physics concepts, such as kinematics, Newton's Laws, and more.",
      });
    ...
    it("should create a valid course record with a published date", async () => {
      ...
      expect(new Date(course.publishedAt).getTime()).toBeCloseTo(
        publishedAt.getTime(),
        -2
      ); // Fix precision issue
    ...
    it("should reject an empty course description", async () => {
      await expect(
        ...
        "Validation error: Course description, if it exists, cannot be empty"
      );
    });
    ...
    it("should reject a description over 500 characters", async () => {
      await expect(
        ...
        "Validation error: Course description must be less than or equal to 500 characters"
      );
    });
    ...
    it("should reject a name over 50 characters", async () => {
      await expect(
        ...
        "Validation error: Course name must be less than or equal to 50 characters"
      );
    });
  });
  ...
  describe("Course.update", () => {
    ...
    it("should update the description", async () => {
      await course.update({
        description: "This is the second course in the physics 20x series.",
      });
      await expect(course.save()).resolves.toBeTruthy();
      await course.reload();
      expect(course.description).toEqual(
        "This is the second course in the physics 20x series."
      );
    });
    ...
    it("should update the publishedAt status", async () => {
      ...
      expect(
        moment(course.publishedAt).utc().isSame(publishedAt, "second")
      ).toBe(true);
    });
